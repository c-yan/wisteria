...
procedure NormalizeFilter(Src: TBitmap; Min, Max: Integer;
  PProc: TProgressProc);
...
function AdobeRGBToXYZ(C: TByteQuad): TSingleQuad; inline;
  ...
  begin
    for I := 0 to 2 do
      Result[I] := Power(C[I] / 255, 2.19921875);
    Result[3] := 0;
  end;
...
function XYZTosRGB(C: TSingleQuad): TByteQuad; inline;
  ...
  begin
    for I := 0 to 2 do
      if C[I] <= 0.00304 then
        Result[I] := Trunc(Clamp(C[I], 0, 1) * 12.92 * 255 + 0.5)
      else
        Result[I] := Trunc((1.055 * Power(Clamp(C[I], 0, 1), 1 / 2.4) - 0.055) *
          255 + 0.5);
    Result[3] := 0;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      U := ((578 * ((SP[X].B shl 4) - L) + 512 + (255 shl 22)) shr 10) -
        (255 shl 12);
      V := ((730 * ((SP[X].R shl 4) - L) + 512 + (255 shl 22)) shr 10) -
        (255 shl 12);
      L := LumaMap[L];
      R := ((L shl 10) + 1436 * V + 8192);
      G := ((L shl 10) - 352 * U - 731 * V + 8192);
      B := ((L shl 10) + 1815 * U + 8192);
      if R > (255 shl 14) then
        DP[X].R := 255
      else if R < 0 then
        DP[X].R := 0
      else
        DP[X].R := R shr 14;
      if G > (255 shl 14) then
        DP[X].G := 255
      else if G < 0 then
        DP[X].G := 0
      else
        DP[X].G := G shr 14;
      if B > (255 shl 14) then
        DP[X].B := 255
      else if B < 0 then
        DP[X].B := 0
      else
...
begin
  if Angle < 0 then
    Angle := Angle mod 360 + 360
  else
    Angle := Angle mod 360;

  if (Src = nil) or Src.Empty or (Angle div 90 = 0) then
    Exit;
  ConvertToTrueColor(Src);
  if not Assigned(PProc) then
    PProc := NullProgressProc;
...
begin
  ...
  Dst.Assign(Src);
  for X := 0 to 255 do
    Table[X] := Clamp(Trunc((X - 127.5) * V + 127.5 + 0.5), 0, 255);
  for Y := 0 to Src.Height - 1 do
  begin
    SP := Src.ScanLine[Y];
    DP := Dst.ScanLine[Y];
...
begin
  ...
  Dst.Assign(Src);
  for X := 0 to 255 do
    Table[X] := Trunc(Power(X / 255, V) * 255 + 0.5);
  for Y := 0 to Src.Height - 1 do
  begin
    SP := Src.ScanLine[Y];
    DP := Dst.ScanLine[Y];
...
begin
  if (Src = nil) or Src.Empty or (Src.Width < T * 2) or (Src.Height < T * 2)
  then
    Exit;
  ConvertToTrueColor(Src);
  if not Assigned(PProc) then
  ...
  for X := 1 to Y do
    Reciprocal[X - 1] := (PV + X shr 1) div X;
...
begin
  ...
  for X := 0 to 4080 do
    LumaMap[X] := Trunc((X - LMin) * LNor + Min + 0.5);
  ...
  begin
    ...
    case Method of
      1:
        for X := 0 to Src.Width - 1 do
        begin
          DP[X] := (B709 * SP[X].B + G709 * SP[X].G + R709 * SP[X].R +
            PH) shr PF;
        end;
      2:
        for X := 0 to Src.Width - 1 do
...
procedure TrimFilterImpl(Src: TBitmap; R: TRect; FillColor: TPixel;
  PProc: TProgressProc);
var
  Dst: TBitmap;
  SP, DP: PPixelArray;
...
begin
  ...
  if R.Right < 0 then
    R.Right := Src.Width + R.Right;
  if R.Bottom < 0 then
    R.Bottom := Src.Height + R.Bottom;
...
procedure IndexedFilter(Src: TBitmap; var Grayscale: Boolean;
  ...
  function CreateLogPalette(Src: TBitmap;
    var LogPalette: TMaxLogPalette): Boolean;
    function MarkBitmap(Src: TBitmap; Map: PByteArray;
      var Grayscale: Boolean): Boolean;
    var
      ColorCount: Integer;
      X, Y: Integer;
      SP: PRGBArray;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Grayscale := Grayscale and (SP[X].B = SP[X].G) and
              (SP[X].B = SP[X].R) and (SP[X].G = SP[X].R);
          end;
        end;
      end;
      Result := True;
    ...
    begin
      LogPalette.palVersion := $0300;
      LogPalette.palNumEntries := 256;
      FillChar(LogPalette.palPalEntry[0], SizeOf(LogPalette.palPalEntry[0])
        * 256, 0);
    end;
    ...
    procedure CreateColorPalette(var LogPalette: TMaxLogPalette;
      Map: PByteArray);
    var
      I, J, X: Integer;
    begin
  ...
  begin
    ...
    try
      ...
      if Grayscale then
        CreateGrayscalPalette(LogPalette)
      else
        CreateColorPalette(LogPalette, Map);
    finally
      FreeMem(Map);
    end;
  end;
  ...
  procedure Create8bitBitmap(Src: TBitmap; var LogPalette: TMaxLogPalette);
    ...
    begin
      ...
      while (C1.B <> LogPalette.palPalEntry[I].peBlue) or
        ...
        (C1.R <> LogPalette.palPalEntry[I].peRed) do
        Inc(I);
      Result := I;
    end;

    function GetIndex(const C1: TRGB; const LogPalette: TMaxLogPalette)
      : Byte; inline;
    var
      N: Integer;
    begin
      N := (C1.B and (LookupCacheValue - 1)) + (C1.G and (LookupCacheValue - 1))
        shl LookupCacheBits + (C1.R and (LookupCacheValue - 1))
        shl (LookupCacheBits * 2);
      if (LookupCache[N].Color.A <> $FF) and (LookupCache[N].Color.B = C1.B) and
        (LookupCache[N].Color.G = C1.G) and (LookupCache[N].Color.R = C1.R) then
      begin
...
begin
  ...
  PProc(0);
  if not CreateLogPalette(Src, LogPalette) then
    Exit;
  Create8bitBitmap(Src, LogPalette);
end;

function SplineInterpolate(T: Double; N: Integer;
  X, Y, Z: PDoubleArray): Double;
var
  I, J, K: Integer;
  D, H: Double;
begin
...
procedure ConvertFromAdobeRGB(Src: TBitmap);
  ...
  begin
    ...
    begin
      T := I / (LUT2Size - 1);
      if T <= 0.00304 then
        LUT2[I] := Trunc(T * 12.92 * 255 + 0.5)
      else
        LUT2[I] := Trunc((1.055 * Power(T, 1 / 2.4) - 0.055) * 255 + 0.5);
    end;
  end;
...
var
  ...
  SP, DP: PByteArray;
begin
  if (Src = nil) or Src.Empty or (Src.PixelFormat <> pf8bit) then
    Exit;
  if not Assigned(PProc) then
    PProc := NullProgressProc;
  ...
  try
    ...
    begin
      ...
      begin
        if (X mod 2) = 0 then
          DP[X div 2] := (SP[X] shl 4) + (DP[X div 2] and $F)
        else
          DP[X div 2] := (DP[X div 2] and $F0) + SP[X];
      end;
      PProc(100 * Y div (Src.Height - 1));
    end;
    Src.Assign(Dst);
